...
public class CirclePacking {
	...
	static Sampler<GraphConfiguration<Circle2D>, BirthDeathModification<Circle2D>> create_sampler(Parameters p, RandomGenerator rng) {
		...
		UniformBirth<Circle2D> birth = new UniformBirth<Circle2D>(rng, new Circle2D(minx, miny, minradius), new Circle2D(maxx, maxy,
				maxradius), builder);
		...
		DirectSampler<Circle2D, GraphConfiguration<Circle2D>, BirthDeathModification<Circle2D>> ds = new DirectSampler<>(distribution,
				birth);
...
//
//	public CirclePacking(File dsmFile, float sigmaD, double energy, double ponderationGrad, double ponderationSurface, double maxsize,
//			double maxratio) throws IOException {
//		OrientedView grad_view = new OrientedPlanarImageWrapperImageIO(dsmFile, sigmaD);
//		ConstantEnergy<Circle2D, Circle2D> c1 = new ConstantEnergy<Circle2D, Circle2D>(energy);
//		ConstantEnergy<Circle2D, Circle2D> c2 = new ConstantEnergy<Circle2D, Circle2D>(ponderationGrad);
//		UnaryEnergy<Circle2D> u1 = new ImageGradientUnaryEnergy<Circle2D>(grad_view);
...
//		ConstantEnergy<Circle2D, Circle2D> c3 = new ConstantEnergy<Circle2D, Circle2D>(ponderationSurface);
//		BinaryEnergy<Circle2D, Circle2D> b1 = new IntersectionAreaBinaryEnergy<Circle2D>();
//		BinaryEnergy<Circle2D, Circle2D> b2 = new MultipliesBinaryEnergy<Circle2D, Circle2D>(c3, b1);
//		// c1 - c2*u1
...
//		GraphConfiguration<Circle2D> conf = new GraphConfiguration<Circle2D>(this.unaryEnergy, this.binaryEnergy);
//		for (int index = 0; index < parameterArray.length;) {
//			BirthDeathModification<Circle2D> modif = new BirthDeathModification<>();
//			Circle2D rectangle = new Circle2D(parameterArray[index++] * this.transform.getMat()[0] + this.transform.getDelta()[0],
//					parameterArray[index++] * this.transform.getMat()[1] + this.transform.getDelta()[1], parameterArray[index++]
//							* this.transform.getMat()[2] + this.transform.getDelta()[2], parameterArray[index++]
//							* this.transform.getMat()[3] + this.transform.getDelta()[3], parameterArray[index++]
...
//		for (int i = 0; i < 5; i++) {
//			val0.set(i, parameterArray[i] * this.transform.getMat()[i] + this.transform.getDelta()[i]);
//		}
...
//		for (int i = 0; i < 5; i++) {
//			result[i] = val1.get(i).doubleValue() * this.transform.getMatInv()[i] + this.transform.getDeltaInv()[i];
//		}
//		return result;
//	}
//
...
//	Sampler<GraphConfiguration<Circle2D>, BirthDeathModification<Circle2D>> samp;
//	Schedule<SimpleTemperature> sch;
//	EndTest end;
//	CompositeVisitor mVisitor;
//
//	public CirclePacking(long seed, File dsmFile, float sigmaD, double energy, double ponderationGrad, double ponderationSurface,
//			double maxsize, double maxratio, double pbirthdeath, double pedge, double pcorner, double psplitmerge, double temp,
...
//		this.conf = (GraphConfiguration<Circle2D>) create_configuration(p, grad_view);
//
