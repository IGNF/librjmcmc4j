...
public class BuildingFootprintRectangleParallelTempering<O extends SimpleObject> {
	...
	static Sampler<GraphConfiguration<Rectangle2D>, BirthDeathModification<Rectangle2D>> create_sampler(
		...
		// new RectangleSplitMergeTransform(), p_split_merge, p_split, 1, 2,
		// "SplitMerge"));
	...
	public static void main(String[] args) {
		 ...
		 * standard output >
		 */
		Visitor[] visitors = new Visitor[nReplicas];
		Visitor visitor = new OutputStreamVisitor(System.out);
		Visitor shpVisitor = new ShapefileVisitor("result",
				new GeometryFilter() {
		...
		visitors[0] = mVisitor;
		Visitor shpVisitorMiddle = new ShapefileVisitor("result_"
				+ (nReplicas / 2), new GeometryFilter() {
			CoordinateFilter coordFilter = new CoordinateFilter() {
				@Override
				public void filter(Coordinate coord) {
					coord.y *= -1;
		...
		Visitor shpVisitorLast = new ShapefileVisitor("result_"
				+ (nReplicas - 1), new GeometryFilter() {
			CoordinateFilter coordFilter = new CoordinateFilter() {
				@Override
				public void filter(Coordinate coord) {
					coord.y *= -1;
	...
	private static Schedule<SimpleTemperature> create_schedule(int i,
			Parameters p) {
		return new GeometricSchedule<SimpleTemperature>(new SimpleTemperature(
				Math.pow(p.getDouble("temp_exponent"), i)),
				p.getDouble("deccoef"));
	}
