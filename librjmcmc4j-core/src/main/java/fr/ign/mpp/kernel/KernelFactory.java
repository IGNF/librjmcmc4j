...
public class KernelFactory<T extends SimpleObject, C extends AbstractGraphConfiguration<T, C, M>, M extends AbstractBirthDeathModification<T, C, M>> {
	 ...
	 */
	public Kernel<C, M> make_uniform_birth_death_kernel(RandomGenerator rng,
			ObjectBuilder<T> builder, UniformBirth<T> b, double p, double q) {
		Kernel<C, M> kernel = new Kernel<C, M>(new NullView<C, M>(),
				new UniformView<T, C, M>(builder), b.getVariate(), new Variate(
	 ...
	 * @return the new kernel
	 */
	public Kernel<C, M> make_uniform_modification_kernel(RandomGenerator rng,
			ObjectBuilder<T> builder, Transform t, double p, String name) {
	 ...
	 */
	public Kernel<C, M> make_uniform_modification_kernel(RandomGenerator rng,
			ObjectBuilder<T> builder, Transform t, double p, double q,
			String name) {
		return make_uniform_modification_kernel(rng, builder, t, p, q, 1, 1,
				name);
	}
	 ...
	 */
	public Kernel<C, M> make_uniform_modification_kernel(RandomGenerator rng,
			ObjectBuilder<T> builder, Transform t, double p, double q, int n0,
			int n1, String name) {
		...
		Kernel<C, M> kernel = new Kernel<>(view0, view1, variate0, variate1, t,
				p, q);
		kernel.setName(name);
		return kernel;
	}
