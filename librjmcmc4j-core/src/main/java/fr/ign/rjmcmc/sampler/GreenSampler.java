...
public class GreenSampler<C extends Configuration<C, M>, M extends Modification<C, M>> implements Sampler<C, M> {
  /**
   * Logger.
   */
  static Logger LOGGER = Logger.getLogger(GreenSampler.class.getName());

  private Density<C, M> density;

  public Density<C, M> getDensity() {
    return density;
  }

  private Acceptance<? extends Temperature> acceptance;
  private Temperature temperature;
  private List<Kernel<C, M>> kernels;
  private AbstractRealDistribution die;
  private double refPdfRatio;
  private double kernelRatio;
  ...
  public GreenSampler(RandomGenerator rng, Density<C, M> d, Acceptance<? extends Temperature> a, List<Kernel<C, M>> k) {
    this.density = d;
    this.acceptance = a;
    this.kernels = k;
  ...
  public void sample(RandomGenerator e, C config, Temperature t) {
    ...
    if (this.greenRatio <= 0) {
      LOGGER.log(Level.FINEST, "NEGATIVE GREEN RATIO " + this.greenRatio + " (" + this.kernelRatio + ", " + this.refPdfRatio + ")");
      this.delta = 0;
      this.accepted = false;
      this.timeDelta = 0;
    ...
    start = end;
    this.acceptanceProbability = this.acceptance.compute(this.delta, this.temperature, this.greenRatio);
    ...
    if (this.accepted) {
      ...
      LOGGER.log(Level.FINEST, "REFUSED " + this.acceptanceProbability + " with " + this.greenRatio + " (" + this.kernelRatio + ", " + this.refPdfRatio + ")");
      this.timeApply = 0;
    }
  }
